from apps.my_admin.models import ProductCategory, Product, ProductImage, User, Media, Article, ArticleCategory,Order, OrderProduct , Message, ProductLanguage, Comment
...
def index(request):
    ...
    lastest_products = Product.objects.all().order_by('-id')[:9]
...
def detail_product(request, id):
    ...
    comments = Comment.objects.filter(product=product).values('user__name', 'content', 'created')

    return render(request, "user/pages/product/detail.html", {"product": data, "related_products": related_products, "comments": comments})
...
def comment(request, product_id):
    if request.method == 'POST':
        ...
        comment = Comment(user=request.user, product=product, content=content)
        comment.save()
...
def get_product_list (request):
    if request.method == "GET":
        ...
        if keyword is not None:
            # Subquery
            ids = ProductLanguage.objects.filter(Q(des__contains=keyword) | Q(name__contains=keyword)).values_list('id', flat=True)
...
def view_orders(request):
    if request.user.is_authenticated:
        ...
        for i in range(0,len(orders)):
            order_products=list(OrderProduct.objects.filter(order=orders[i].id))
            total_order=0
            for j in range(0,len(order_products)):
                # tính tổng giá trị đơn hàng
                total_order=total_order+order_products[j].product.cost*order_products[j].quantity
...
def shopping_cart (request):
    if request.method == "GET":
        ...
        for product in products:
            ...
            total += int(product_data.cost) * int(product['quantity'])
...
def checkout (request):
    ...
    if request.method == "POST":
        ...
        for product in data['products']:
            product_data = Product.objects.filter(pk=product['id']).first()
            order_product = OrderProduct()
            order_product.quantity = product['quantity']
            order_product.order = order
            order_product.product = product_data
            total += int(product_data.cost) * int(product['quantity'])
            order_product.save()
