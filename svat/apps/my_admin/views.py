...
from apps.my_admin.models import Article, Language, ArticleCategory, User, ProductCategory, Media, ArticleLanguage, ProductImage, ProductLanguage, Product, Order, OrderProduct,ProductLanguage,ImportOrderProduct,ImportOrder, Message
...
def get_products(request):
    values = ['id', 'cost', 'quantity', 'view', 'des', 'productlanguage__name']
    order_fields = ['-id']
...
def update_order(request):
    if request.method == 'POST':
        try:
            ...
            if status == 4:
                detail_orders = list(OrderProduct.objects.filter(order=order_id).values('id', 'quantity', 'product').all())
                for ele in detail_orders:
                    product = Product.objects.filter(pk=ele['product']).first()
...
def get_detail_order(request):
    if request.method == 'POST':
        ...
        order = Order.objects.filter(pk=order_id).values('id', 'name', 'address', 'email', 'phone_number').first()
        detail_orders = list(OrderProduct.objects.filter(order=order_id).values('id', 'quantity', 'product').all())
        data = {
            "order": order,
            "detail_orders": []
        ...
        for ele in detail_orders:
            product = Product.objects.filter(pk=ele['product']).values('id', 'productlanguage', 'cost').first()
            product_name = ProductLanguage.objects.filter(pk=product["productlanguage"]).values('name').first()
...
def view_detail(request):
    if request.method == "POST":
        ...
        import_order_products= list(ImportOrderProduct.objects.filter(import_order=req['id']).values('quantity','product'))
        len_import_order=len(import_order_products)
        for i in range(0,len_import_order):
            import_order_products[i]['id']=import_order_products[i]['product']       
            import_order_products[i]['product']=ProductLanguage.objects.get(product=int(import_order_products[i]['product']),language=1).name
            import_order_products[i]['import_order']=req['id']
        data={
            'data':import_order_products,
            'success':True,
        }
        return HttpResponse(json.dumps(data), content_type='text/json')
...
def create_import_order(request):
    if request.method == "POST":
        ...
        id_user=User.objects.get(email=str(request.user))

        newImportOrder=ImportOrder(
            created=time_created,
            user=id_user,
        )
        newImportOrder.save()

        id_import_order=ImportOrder.objects.get(created=time_created)
        for i in range(0,len(req['data'])):
            id_product=int(req['data'][i]['id'])
            quantity=int(req['data'][i]['quantity'])

            import_order_product=ImportOrderProduct(
                quantity=quantity,
                import_order=id_import_order,
...
def get_all_product(request):
    if request.method == "GET":
        ...
        for i in range(0,len(products)):
            products[i]['text']=ProductLanguage.objects.get(product=products[i]['id'],language=1).name
...
def get_all_message(request):
    if request.method == 'GET':
        values = ['id', 'name', 'email', 'message', 'created']
